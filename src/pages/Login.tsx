        ...
        <div className="bg-white py-8 px-4 shadow-xl shadow-slate-200/50 sm:rounded-none sm:px-10 border border-slate-100">
          ...
          <div className="mt-6">
             ...
             <div className="mt-6 grid grid-cols-2 gap-3 text-xs text-slate-500 text-center">
                <div 
                  className="p-2 bg-slate-50 rounded border border-slate-100 cursor-pointer hover:bg-slate-100 transition-colors" 
                  onClick={() => { setEmail('customer@lumina.store'); setPassword('password'); }}
                >
                   Customer<br/><span className="opacity-50">customer@lumina.store</span>
                </div>
                <div 
                  className="p-2 bg-slate-50 rounded border border-slate-100 cursor-pointer hover:bg-slate-100 transition-colors" 
                  onClick={() => { setEmail('admin@lumina.store'); setPassword('password'); }}
                >
                   Admin<br/><span className="opacity-50">admin@lumina.store</span>
                </div>
             </div>
          </div>
